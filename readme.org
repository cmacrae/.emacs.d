...
** Set meta for Darwin systems & turn on menu-bar mode
   I use the Emacs "Mac port" package, as it has much better GUI support on Darwin systems.
   As such, it's necessary to tell it to use the meta key as the option modifier.
   Also, to make it play a little nicer with window management, enable menu-bar-mode.
   #+begin_src emacs-lisp
   (cond
